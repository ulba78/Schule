/* Schulkalender Editor – Feature-Branch + PR
   Einfache Web-App zum Bearbeiten einer ICS-Datei direkt im GitHub-Repo.
   Voraussetzungen:
   - GitHub PAT mit Repo-Contents-Write (oder fine-grained Token mit "Contents: Read/Write")
...
async function createPR(owner, repo, title, head, base, body){
  const url = `https://api.github.com/repos/${owner}/${repo}/pulls`;
  return await ghPostJson(url, { title, head, base, body });
}
...
async function doSave(){
  setStatus('Erstelle Feature-Branch und PR...');
  const owner = UI.owner.value.trim(); const repo = UI.repo.value.trim();
  const path = UI.path.value.trim(); const base = UI.baseBranch.value.trim();
  ...
  try{
    ...
    // 5) PR erstellen
    const prTitle = 'Kalender-Update (.ics) – Web-Editor';
    const prBody = `Automatisch erstellt vom Web-Editor am ${new Date().toLocaleString(CONFIG.LOCALE)}.\n\n- Anzahl Termine: ${events.length}\n- Datei: ${path}\n- Branch: ${featureBranch}`;
    const pr = await createPR(owner, repo, prTitle, featureBranch, base, prBody);

    setStatus(`Gespeichert. PR erstellt: #${pr.number}`);
    alert(`PR erstellt: #${pr.number}\n${pr.html_url}`);
  }catch(e){
    console.error(e);
